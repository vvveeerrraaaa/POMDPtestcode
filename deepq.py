...
class Model(DQN_Agent):
    ...
    def prep_minibatch(self):
        ...
        # rward.append(sum(sum(batch_reward)).item())
        # # print((sum(sum(batch_reward))).item())
        # # print(rward)
...
for frame_idx in range(1, config.MAX_FRAMES + 1):
    ...
    if done:
        ...
        if np.mean(model.rewards[-10:]) > 19:
            plot(frame_idx, model.rewards, model.losses, None, timedelta(seconds=int(timer()-start)))
            break
